...
/*   Updated: 2024/09/22 16:58:46 by hucherea         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool	is_in_window(const long number, const t_window *window)
{
...
void	sort_by_window(t_stacks *stack, t_window *window)
{
	while (stack->a->size > SMALL_STACK)
	{
		...
		else if (stack->a->list[0] < window->min)
		{
			push_min_windows(stack, window);
		}
		else
		{
			if (stack->a->list[stack->a->size - 1] <= window->max)
